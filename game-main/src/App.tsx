...
const fallbackConfig: SiteConfig = {
  ...
  announcement:'15 ngôn ngữ • 40+ Game Modes • bài tập thực tế', maintenance:false,
  primaryCta:'Chơi ngay', footerAbout:'Sky First Games là nền tảng trò chơi học tập đa ngôn ngữ thuộc hệ sinh thái Sky First Network.',
  seoTitle:'Sky First Games', seoDescription:'Hệ sinh thái trò chơi học tập đa ngôn ngữ của Sky First.', contactEmail:'hotro.sfn@gmail.com'
};
...
function DynamicPlay({modeId,setId}:{modeId:string|null;setId:string|null}){
  const [mode,setMode]=React.useState(gameModes.find(m=>m.id===modeId)||gameModes[0]);
  const [set,setSet]=React.useState(seedQuestionSets.find(s=>s.id===setId)||seedQuestionSets[0]);
  React.useEffect(()=>{
    api.publicGameModes().then(r=>{const m=r.modes.find(x=>x.id===modeId);if(m)setMode(m)}).catch(()=>{});
    api.publicQuestionSets().then(r=>{const q=r.sets.find(x=>x.id===setId);if(q)setSet(q)}).catch(()=>{});
  },[modeId,setId]);
  return <GamePlayer set={set} mode={mode}/>;
}
...
export default function App(){
  ...
  else if(p==='/play') page=<DynamicPlay modeId={route.query.get('mode')} setId={route.query.get('set')}/>;
