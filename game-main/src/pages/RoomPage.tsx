...
export function RoomPage(){
 ...
 React.useEffect(()=>{if(!code){setErr('Thiếu mã trò chơi.');return;}api.publicGame(code).then(setData).catch(e=>setErr(e.message))},[code]);
 ...
 if(err)return <main className="auth-page"><div className="auth-card"><h1>Không vào được game</h1><p>{err}</p></div></main>;
 if(!data)return <main className="auth-page"><div className="auth-card"><Loader2 className="spin"/><h1>Đang tải trò chơi…</h1></div></main>;
