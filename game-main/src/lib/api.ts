...
export const api = {
  ...
  hostPass: (token: string) => request<{ pass: any; game: any; set: QuestionSet }>(`/api/host/pass/${encodeURIComponent(token)}`),
  startHostSession: (token: string) => request<{ session: any }>(`/api/host/pass/${encodeURIComponent(token)}/sessions`, { method: 'POST' }),
  ...
  createPass: (payload: unknown) => request<{ pass: any }>('/api/creator/access-passes', { method: 'POST', body: JSON.stringify(payload) }),
